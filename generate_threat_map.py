...
SHODAN_API_KEY = os.environ.get("SHODAN_API_KEY", "K8eOKHhxQZCNGtHv2gSJEYZAlEaND1XC")
...
def get_geolocations_batch(ips):
    ...
    try:
        response = requests.post("http://ip-api.com/batch?fields=lat,lon,country,countryCode,query,status,isp,org,as,city", json=ips, timeout=15)
        response.raise_for_status()
